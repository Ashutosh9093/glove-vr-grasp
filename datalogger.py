...
Now logs the complete task sequence phase in real-time.
"""
...
class DataLogger:
    def __init__(self, out_dir='logs', filename=None, num_flex=5):
        ...
        # IMU inertial data
        header += ["gx", "gy", "gz", "ax", "ay", "az"]

        # Hand position tracking
        header += ["hand_pos_x", "hand_pos_y", "hand_pos_z"]

        # Translational movement
        header += ["hand_vel_x", "hand_vel_y", "hand_vel_z"]
        header += ["hand_displacement"]

        # Grasp information
        header += ["grasp_amount", "finger_contacts"]

        # Table transition detection
        header += ["table_transition"]

        # RECTIFIED v4.0: Task phase logging
    ...
    def log_sample(self, timestamp, millis, flex_raw, flex_smooth, flex_norm=None,
        ...
        with self.lock:
            ...
            # RECTIFIED v4.0: Task phase and elapsed time
